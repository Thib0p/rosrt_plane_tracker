...
public:
  cameraToRobot()
  {
    Rx=AffineTransformation(0,0,0,-(90-25)*M_PI*2/360, 0, 0);
    camera=AffineTransformation(0,0,0,0.436, 0, 0);
    cameraNeck=AffineTransformation(-0.07,-0.205, -0.02,0, 0, 0);
    ...
    Eigen::Matrix3d eigentransfo= targetQuat.toRotationMatrix();
  ...
  apply(marker myMarker, Vector out)
  {
    out = transfon.apply(myMarker.x,myMarker.y,myMarker.z);
    matToQuat(Rx.getMatrix(),myMarker.xq,myMarker.yq,myMarker.zq,myMarker.wq);
  }
private:
...
run_rosrt_plane_tracker(void)
{
  ...
  cart[1] = Kp*(cart_state[1].x[1]-out[1]);
  cart[2] = Kp*(cart_state[1].x[2]-out[2]);
  cart[3] = Kp*(cart_state[1].x[3]-out[3]);
